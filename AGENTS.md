...
<project_safety>
小程序产物安全红线：
- 杜绝 `api.example.com` 进入小程序产物。
- 千万不要为了测试或验证而编译出包含 `api.example.com` 的小程序产物。
- 构建前必须确认配置不会落到 `api.example.com`。
- 构建后的校验必须挡掉 `api.example.com`。
</project_safety>
...
- 改变公共行为却不更新文档。
- 写与代码现实不一致的文档。
- 让 `api.example.com` 出现在任何小程序构建产物中。
</forbidden>
