...
- 历史 Figma 设计稿已于 2026-08 废弃并删除（`figma设计图/` 不再存在）；视觉以本文档设计 token 与 `@juben-sha/miniapp-ui` 组件体系为准
- 用户提供的 Material You 设计系统 prompt

范围优先级：

1. 第一版功能范围以 `docs/prd-v1.md` 和 `docs/technical-spec-v1.md` 为准。
2. 小程序首版只做首页、对话、社区、我的四个底部 tab（当前实现为「首页 / 聊天 / 社区 / 我的」，V1 命名“对话”已改“聊天”）。
3. 首版不做群聊、Galgame、复杂表情包、完整社区内容发布、宿舍、正式实名、防沉迷。
4. 界面细节以本文档和现有 `@juben-sha/miniapp-ui` 组件实现为准，尤其是底部导航、页面结构、角色页和对话页细节（历史 Figma 定稿已废弃）。
...
本节规范源自 2026-06-14 的 Figma 定稿；原始 PNG（`figma设计图/`）已于 2026-08 废弃并删除。以下设计 token、组件规范与文案中文化清单**继续有效**，是当前小程序视觉体系的设计依据；不得另起一套视觉风格，也不得把整页 PNG 当页面背景替代真实 UI 组件。
...
- `BottomTabBar`：当前实现为首页、聊天、社区、我的四个 tab。任何宿舍、语音、图片入口不得加入当前版本。
