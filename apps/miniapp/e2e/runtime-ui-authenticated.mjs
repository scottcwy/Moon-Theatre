...
const PAGE_CHECKS = [
  ...
  },
  {
    ...
    run: async (miniProgram, page) => {
      ...
      // 列表点击角色 → 自由会话（getCharacterChatUrl 一律 mode=free），留言正文直接可见、恰好一次。
      const scopeLabel = await chatPage.$('.chat-page__scope-label');
      const scopeText = scopeLabel ? await scopeLabel.text().catch(() => '') : '';
      assert(scopeText === '自由聊天', `Expected 自由聊天 scope label after list entry, got ${scopeText || 'none'}`);
      let bubbles = await chatPage.$$('.chat-bubble__text');
      let texts = await Promise.all(bubbles.map((bubble) => bubble.text().catch(() => '')));
      const freeHits = texts.filter((text) => text.includes('回来吧，庭院的花开了一夜。')).length;
      assert(freeHits === 1, `Expected the return message exactly once in free-mode history, got ${freeHits}`);

      // 切到剧本模式（availableModes = ['script', 'free']）：剧本会话历史不含留言。
      ...
      let scriptTexts = [];
      let scriptReached = false;
      while (Date.now() < scriptDeadline) {
        const scriptScope = await chatPage.$('.chat-page__scope-label');
        const scriptScopeText = scriptScope ? await scriptScope.text().catch(() => '') : '';
        if (scriptScopeText === '剧本模式') {
          scriptReached = true;
          bubbles = await chatPage.$$('.chat-bubble__text');
          scriptTexts = await Promise.all(bubbles.map((bubble) => bubble.text().catch(() => '')));
          if (scriptTexts.some((text) => text.includes('布雷诺'))) break;
        }
        await chatPage.waitFor(250);
      }
      assert(scriptReached && scriptTexts.length > 0, 'Expected script-mode history to load after mode switch');
      assert(
