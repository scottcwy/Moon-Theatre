...
- 历史角色或剧本下架时仍可查看最近入口和消息，但 `canSend=false`。

## 3. 服务端读取模型

新增 `GET /api/chat/characters`。服务端按当前用户的 `characterId` 聚合 Chat Session，在聚合结果中选择 `updatedAt` 最新的一条作为摘要来源，并在聚合后执行搜索和分页。
...
每一行展示角色头像、角色名、最近消息、更新时间和必要的只读状态。点击行后携带 `latestSessionId` 进入聊天页，聊天页从会话元数据恢复 `mode`、`scriptId` 和 Visible History。
...
- 最近会话已下架：允许打开历史，聊天页按 `canSend=false` 进入只读态。
...
- API 测试证明每个角色最多一项、最近模式选择正确、聚合后搜索和分页正确、retired 历史可见且只读。
- 列表模型测试证明不含模式筛选，点击使用 `latestSessionId`，同一角色不会由客户端逻辑产生重复展示。
- 聊天页测试证明默认打开最近模式，切换后分别加载两套历史，发送参数和上下文不跨模式。
