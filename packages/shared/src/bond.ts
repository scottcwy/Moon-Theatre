/**
 * 羁绊等级模型（纯逻辑，跨端共享：miniapp-ui 展示层、miniapp 聊天反馈、api prompt 共用）。
 * 服务端数据模型保持 `bondLevel`（1–10，每 100 经验升 1 级、封顶 10）/`bondExp`（累计经验）不变；
 * 前端展示层按下方 6 级累计经验门槛重算层级与名称，不直接消费服务端 `bondLevel`。
 */
 ...
 * 花完 1000 点（≈10000 exp）最多升到 4 级「留盏」，达不到 5 级「不言」。
 */
export const BOND_LEVEL_THRESHOLDS = [0, 200, 700, 2700, 10700, 26700] as const;
...
/** 累计经验 → 6 级档位（1–6）。 */
export function bondLevelFromExp(totalExp: number): number {
  let level = 1;
...
export function createBondViewModel(input?: BondRelationshipInput | null): BondViewModel {
  ...
  return buildViewModel(bondLevelFromExp(totalExp), totalExp);
}
